...
hexo.extend.helper.register('list_localized_archives', function listArchivesHelper(options) {
  options = options || {};

  var config = this.config;
  var archiveDir = config.archive_dir;
  var timezone = config.timezone;
  var lang = options.lang || this.page.lang || this.page.language || config.language[0];
  ...
  function link(item) {
    ...
    if (config.language[0] !== lang) {
      url = lang + '/' + url;
    } else {
      url = config.language[0] + '/' + url;
    }
